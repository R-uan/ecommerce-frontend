...
export default function ProductMiniature({ data }: { data: IProductsPartial }) {
	...
	return (
		<Link className="w-fit h-fit" href={`/catalog/${data.id}`}>
			<div className="flex flex-col w-[270px] rounded-md min-h-[30.5vw] h-fit">
				...
				<div className="flex flex-col items-center justify-between h-[clamp(70px,5.5vw,5.5vw)] m-0.5 p-[5px] rounded-[0px_0px_5px_5px]">
					<div className="gap-[5px] w-[260px] flex items-center flex-col">
						<h3 className="text-[clamp(1rem,1.25vw,1.25vw)] break-words text-center leading-[clamp(1rem,1.25vw,1.25vw)] font-kode font-bold">
							{data.name}
						</h3>
						<h6 className="text-[clamp(0.65rem,1.2vw,1.2vw)] leading-[clamp(0.65rem,1.2vw,1.2vw)] text-center font-kode">
							{data.manufacturer}
						</h6>
